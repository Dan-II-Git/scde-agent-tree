...
for d in all_dists:
    ...
    if adm and adm > 0:
        ...
        sac_pp = round(la.get('taxes_fees', 0.0) / adm)
        additional_pp = round(la.get('additional', 0.0) / adm)
        dist_svc_pp = round(la.get('dist_svc', 0.0) / adm)
        invest_pp = round(la.get('invest', 0.0) / adm)
