...
for d in all_dists:
    ...
    if hc and hc > 0:
        ...
        if is_reported:
            sac_pp       = round(la.get('local_sac_req', 0.0)    / hc)
            additional_pp= round(la.get('local_additional', 0.0) / hc)
            dist_svc_pp  = round(la.get('local_dist_svc', 0.0)   / hc)
            invest_pp    = round(la.get('local_investments', 0.0) / hc)
